  ...
  return (
    <section id="sports" className="py-[100px] px-[5%] bg-white">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(185px,1fr))] max-md:grid-cols-2 max-[480px]:grid-cols-2 gap-4 max-w-[1200px] mx-auto">
        {sports.map((s) => (
          <Link key={s.sport_id} href={`/register/${s.sport_id}`}
            className={`reveal rounded-[18px] p-7 max-[480px]:p-5 text-center cursor-pointer relative overflow-hidden shadow-[0_2px_16px_rgba(27,48,145,0.10)] transition-all duration-300 no-underline group border-2
              ${s.is_active
                ? 'bg-white border-[rgba(27,48,145,0.12)] hover:-translate-y-2 hover:shadow-[0_8px_30px_rgba(27,48,145,0.25)] hover:border-navy'
